...
if($_POST["usuarioAceptaPerfil"]){

	$INTRANET->Execute("
		UPDATE SGI_Empleados 
		set 
		xd_aceptacionPerfilPuesto = NOW()
		WHERE kf_usuario = '".$_POST["usuarioAceptaPerfil"]."' 
	");
...
if($_GET["usuario"]){
	$datosPersonal= $INTRANET->getRow("SELECT *, (SELECT xt_puesto FROM perfiles.usuarios WHERE xt_usuario = kf_usuario) as xt_perfil_puesto FROM SGI_Empleados WHERE kf_usuario='".$_GET["usuario"]."' ORDER BY xn_Foto ");
}else{
	$datosPersonal= $INTRANET->getRow("SELECT *, (SELECT xt_puesto FROM perfiles.usuarios WHERE xt_usuario = kf_usuario) as xt_perfil_puesto FROM SGI_Empleados WHERE kf_usuario='".$_SESSION["sesionUsuario"]."' ORDER BY xn_Foto ");
}
...
echo"<div id='wrapperFicha' align='center'>";
	...
	echo "</table>"; 
		...
		echo "<table border='0' align='center' width='600'>";
			...
			echo"<tr>";
				echo"<td id='titulo2'>Perfil de puesto</td>";
				echo "<td>".$datosPersonal["xt_Empresa"]."</td>";
				if(date('Y') == date('Y', strtotime($datosPersonal["xd_aceptacionPerfilPuesto"]))){
					echo "<td>".$datosPersonal["xd_aceptacionPerfilPuesto"]."</td>";
				}
			echo"</tr>";
		///Linea
			echo "<tr>";
			...
			echo "<tr height='10'>";
				...
				echo '
				<object
					data="./perfiles_puestos/'.$datosPersonal["xt_perfil_puesto"].'.pdf"
				...
				>
					...
					document]('.$datosPersonal["xt_perfil_puesto"].')
					</p>
				</object>
				';
				echo "</td>";
		...
		echo "<table border='0' align='center' width='600'>";
			...
			if( ($datosPersonal["xd_aceptacionPerfilPuesto"] == null) || (date('Y') != date('Y', strtotime($datosPersonal["xd_aceptacionPerfilPuesto"]))) ){
				echo"<tr height='40'>";
					echo"<td colspan='2' valign='bottom' align='right'>";
						echo "<form  method='POST' action='perfilPuesto.php'>";
							echo "<input type= 'hidden' name='usuarioAceptaPerfil' value='".$datosPersonal["kf_usuario"]."' >";
